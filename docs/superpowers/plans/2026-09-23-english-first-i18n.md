...
| 2026-09-23 | Phase 4 wave 1 (part) — onboarding + auth English | feat/i18n-en-onboarding | 463 keys in `en.json`: `app/onboarding/**`, `app/(public)/{login,signup}`, `app/auth`, `components/auth/*`, WhatsApp pairing/channel components. `en` still hidden (`em_construcao`), so not visible in UI yet; shell/nav and shared UI (Wave 1 rest) not translated. Note: fresh `pnpm typecheck` OOMs at Node default heap (~3.1 GB) on this machine — also on upstream main — use `NODE_OPTIONS=--max-old-space-size=8192` locally. |
| 2026-09-24 | Phase 3 tooling + Phase 4 (all waves) + Phase 6 (partial) | feat/i18n-en-waves | `en.json` now 7,4k keys, 100% of UI `t()` keys (6 independent review slices applied, 79 fixes); `en` promoted to `completo` and therefore visible in top bar / profile / organization; English opt-out vocabulary added (`lib/opt-out/deteccao.ts` + tests); bootstrap accepts `en` and defaults to it; installer option 3 = English (default). **Not done:** Phase 5 non-dictionary surfaces (API errors, e-mail templates, notifications, AI prompt scaffolding, seed pipeline names), browser proof (port 3128 is held by another session's dev server), `IDIOMA_PADRAO` flip, e2e, `.changes/` fragment. |
