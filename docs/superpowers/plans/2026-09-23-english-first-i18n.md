...
## 0. Facts measured on 2026-09-23 (main@0a47326b3) — re-measure, don't trust
...
| Dictionary is one ~8.7k-line TS file with ~5,570 `es` entries | `lib/i18n/dicionario.ts` | `grep -c ': { es:' lib/i18n/dicionario.ts` |
| `lib/i18n/traducoes/zh-CN.json` exists (flat `{ "pt text": "translation" }`) but **no code loads it yet** | `lib/i18n/traducoes/` | `grep -rn 'traducoes/' lib app components --include=*.ts*` |
| Client hook `useT()`; ~418 `.tsx` files call `t("...")` | `lib/i18n/IdiomaProvider.tsx` | `grep -rlE '\bt\("' app components --include=*.tsx \| wc -l` |
...
- **D4 — Customer-facing messages (sent to leads over WhatsApp/e-mail) follow the
  organization locale**, not the viewer's UI locale.
...
- [ ] Sync: `git merge upstream/main` into `main`, push to `origin`.
- [ ] Run the upstream check in §0. Update §0 table with fresh numbers.
- [ ] Read fully: `lib/i18n/*.ts`, `lib/i18n/IdiomaProvider.tsx`,
...
- [ ] Write `scripts/i18n-inventario.ts` (or reuse the gate's AST walker) that prints: total
      distinct `t()` keys, keys in `DICIONARIO`, keys per screen/route group. Output to
      `docs/i18n/inventory-en.md`.
- [ ] Inventory the non-dictionary surfaces listed in §0 with counts per file.
